 ...
 * @params treeSort(oldArr, name [, asynchronous] [, isMax]) 参数必须按顺序传递
 * @oldArr 需要排序的数组数据 【必传】
 * @name . 你要以哪个属性排序？例如 treeSort([{id:2},{id:1},{id:3}],'id')【必传】
 * @asynchronous 可选，是否开启异步，返回一个promise，true为开，默认值false
 * @isMax  可选，true为从大到小，false为从小到大，默认值false
 * @returns treeSort会返回一个全新的数组
 * @author Myway
 */
 export default function treeSort(oldArr, name, asynchronous=false, isMax=false) {
  if(asynchronous) {
    // 异步模式
